...
int testing(istream& in){
	in.tie(0);
	state S;

	string ignored;
	in >> ignored;
	if(ignored == "exit") return 0;
	in >> ignored;
	int Numvars; in >> ignored >> Numvars; S.rsNumvars(Numvars);
	
	in >> ignored;
	numbering<string> vars;
	for(int i = 0; i < S.numvars; i++){
		string str; in >> str; vars(str);
	}

	in >> ignored;
	for(auto &x : S.vars) in >> x;
	in >> ignored;
	for(int i = 0; i < S.numvars; i++){int a; in >> a; S.fixed[i] = a;}

	int N; in >> ignored >> N; S.rsN(N);
	...
	for(int i = 0; i < S.n; i++){
		...
		string str; in >> ignored >> str;
		while(true){
			if(str == "POLY" || str == "DONE") break;
			if(str == "+"){in >> str; continue;}
			if(str == "-"){in >> str; coeff *= -1; continue;}
